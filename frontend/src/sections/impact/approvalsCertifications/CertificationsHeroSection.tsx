...
// Interactive Credentials Mockup Overlay within Visual Frame
const InteractiveCredentialPreview = styled(Box)(({ theme }) => ({
  position: 'absolute',
...
const TRUST_CREDENTIALS = [
  {
    ...
    title: '80G Registered Trust',
    detail: 'WinVinaya Foundation is a registered charitable trust with formal 80G tax exemption status.',
  },
  {
    icon: WorkspacePremiumOutlinedIcon,
    tone: 'accent' as const,
    title: 'IAAP Certified Auditors',
    detail: 'Auditors credentialed by the International Association of Accessibility Professionals (CPACC & WAS).',
  },
  {
    icon: MenuBookOutlinedIcon,
    tone: 'info' as const,
    title: '8 Recognized Global Standards',
    detail: 'Benchmarked against WCAG 2.1/2.2, Section 508, GIGW 3.0, PDF/UA, and SEBI regulatory circulars.',
  },
]

export const CertificationsHeroSection = () => (
  <Root aria-labelledby="certifications-hero-heading">
      ...
      <Grid container spacing={{ xs: 5, md: 6, lg: 8 }} sx={{ alignItems: 'center' }}>
        ...
        <Grid size={{ xs: 12, md: 7, lg: 6 }}>
          <Stack spacing={3.5}>
            <Chip
              icon={<PulseDot />}
              label={approvalsCertificationsHeroData.eyebrow}
              variant="outlined"
              size="medium"
              sx={(theme) => ({
            ...
            >
              {approvalsCertificationsHeroData.headline} <GradientText>{approvalsCertificationsHeroData.headlineHighlight}</GradientText>
            </Typography>
            ...
            >
              {approvalsCertificationsHeroData.subheadline}
            </Typography>

            {/* Rigor & Distinction Note */}
            <Box
              sx={(theme) => ({
                p: 1.75,
            ...
            >
              ...
              <Typography variant="caption" color="text.secondary" sx={{ lineHeight: 1.5, fontSize: '0.8125rem' }}>
                {approvalsCertificationsHeroData.disclaimer}
              </Typography>
            </Box>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ pt: 1 }}>
              <Button
                tone="primary"
                variant="contained"
                href={approvalsCertificationsHeroData.primaryCta.href}
                size="large"
              ...
              >
                {approvalsCertificationsHeroData.primaryCta.label}
              </Button>

              <Button
                variant="outlined"
                size="large"
                href={approvalsCertificationsHeroData.secondaryCta.href}
                sx={(theme) => ({
                  px: 3.5,
                  py: 1.6,
                  fontSize: '1.0625rem',
              ...
              >
                {approvalsCertificationsHeroData.secondaryCta.label}
              </Button>
            </Stack>

            {/* Key Stat Pills */}
            <Stack direction="row" spacing={1.5} sx={{ pt: 1, flexWrap: 'wrap', gap: 1.5 }}>
              {approvalsCertificationsHeroData.stats.map((stat, idx) => (
        ...
        <Grid size={{ xs: 12, md: 5, lg: 6 }}>
          <Box sx={{ position: 'relative', px: { xs: 0, sm: 2, md: 0 } }}>
            <VisualFrame>
              <VisualImage
                role="img"
                aria-label="WinVinaya IAAP certified auditors and registered trust approvals overview."
              >
                {/* Live Credentials Matrix Snapshot Card */}
                <InteractiveCredentialPreview>
                  <Stack direction="row" spacing={1} sx={{ alignItems: 'center', justifyContent: 'space-between' }}>
                    <Stack direction="row" spacing={0.75} sx={{ alignItems: 'center' }}>
                      <SecurityOutlinedIcon sx={{ fontSize: 20, color: 'primary.main' }} />
                  ...
                  <Grid container spacing={1} sx={{ pt: 0.5 }}>
                    ...
                    <Grid size={{ xs: 6 }}>
                      <Stack direction="row" spacing={0.75} sx={{ alignItems: 'center' }}>
                        <WorkspacePremiumOutlinedIcon sx={{ fontSize: 16, color: 'accent.main' }} />
                        <Typography variant="caption" sx={{ fontWeight: 600, fontSize: '0.75rem' }}>
                    ...
                    <Grid size={{ xs: 6 }}>
                      <Stack direction="row" spacing={0.75} sx={{ alignItems: 'center' }}>
                        <MenuBookOutlinedIcon sx={{ fontSize: 16, color: 'accent.main' }} />
                        <Typography variant="caption" sx={{ fontWeight: 600, fontSize: '0.75rem' }}>
                          WCAG 2.1 / 2.2 AA
                        </Typography>
                      </Stack>
            ...
            >
              ...
              >
                <WorkspacePremiumOutlinedIcon sx={{ fontSize: 26 }} />
              </Box>
              <Box>
                <Typography variant="subtitle2" sx={{ fontWeight: 800, color: 'text.primary', lineHeight: 1.2 }}>
                  IAAP Certified Team
