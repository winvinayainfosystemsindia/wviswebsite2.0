...
const VisualImage = styled(Box)(({ theme }) => ({
  ...
  '&::after': {
    ...
    position: 'absolute',
    inset: 0,
    background: `linear-gradient(180deg, transparent 55%, ${alpha(theme.palette.common.black, 0.45)} 100%)`,
  },
}))

// Interactive Mockup Overlay within Visual Frame
const InteractiveDocPreview = styled(Box)(({ theme }) => ({
  position: 'absolute',
...
const TRUST_CREDENTIALS = [
  {
    ...
    title: 'Proprietary Conversion Engine',
    detail: 'Converts source PDFs directly into structured, standards-compliant Word or PDF in minutes, eliminating manual bottlenecks.',
  },
  {
    icon: RecordVoiceOverOutlinedIcon,
    tone: 'accent' as const,
    title: 'Expert Human QA & PwD Validation',
    detail: 'Every automated output undergoes rigorous specialist review and native screen reader testing before delivery.',
  },
  {
    icon: SchoolOutlinedIcon,
    tone: 'info' as const,
    title: 'STEM & Academic Specialists',
    detail: 'Complex data tables, mathematical notation, footnotes, and regional languages preserved with 100% precision.',
  },
]

export const RemediationHeroSection = () => (
  <Root aria-labelledby="remediation-hero-heading">
      ...
      <Grid container spacing={{ xs: 5, md: 6, lg: 8 }} sx={{ alignItems: 'center' }}>
        ...
        <Grid size={{ xs: 12, md: 7, lg: 6 }}>
          <Stack spacing={3.5}>
            <Chip
              icon={<PulseDot />}
              label={remediationHeroData.eyebrow}
              variant="outlined"
              size="medium"
              sx={(theme) => ({
            ...
            >
              {remediationHeroData.headline} <GradientText>{remediationHeroData.headlineHighlight}</GradientText>
            </Typography>
            ...
            >
              {remediationHeroData.subheadline}
            </Typography>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ pt: 1 }}>
              <Button
                tone="primary"
                variant="contained"
                href={remediationHeroData.primaryCta.href}
                size="large"
              ...
              >
                {remediationHeroData.primaryCta.label}
              </Button>

              <Button
                variant="outlined"
                size="large"
                href={remediationHeroData.secondaryCta.href}
                sx={(theme) => ({
                  px: 3.5,
                  py: 1.6,
                  fontSize: '1.0625rem',
              ...
              >
                {remediationHeroData.secondaryCta.label}
              </Button>
            </Stack>

            {/* Key Stat Pills */}
            <Stack direction="row" spacing={1.5} sx={{ pt: 1, flexWrap: 'wrap', gap: 1.5 }}>
              {remediationHeroData.stats.map((stat, idx) => (
        ...
        <Grid size={{ xs: 12, md: 5, lg: 6 }}>
          <Box sx={{ position: 'relative', px: { xs: 0, sm: 2, md: 0 } }}>
            <VisualFrame>
              <VisualImage
                role="img"
                aria-label="Document accessibility specialists converting PDFs into structured, screen-reader compatible formats."
              >
                {/* Live Process Transformation Card */}
                <InteractiveDocPreview>
                  <Stack direction="row" spacing={1} sx={{ alignItems: 'center', justifyContent: 'space-between' }}>
                    <Stack direction="row" spacing={1} sx={{ alignItems: 'center' }}>
                      <PictureAsPdfOutlinedIcon sx={{ fontSize: 20, color: 'primary.main' }} />
                      <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.825rem' }}>
                        Source PDF → Accessible Output
                      </Typography>
                    </Stack>
                    <Box
                      sx={(theme) => ({
                  ...
                  <Grid container spacing={1} sx={{ pt: 0.5 }}>
                    <Grid size={{ xs: 6 }}>
                      <Stack direction="row" spacing={0.75} sx={{ alignItems: 'center' }}>
                        <ArticleOutlinedIcon sx={{ fontSize: 16, color: 'accent.main' }} />
                        <Typography variant="caption" sx={{ fontWeight: 600, fontSize: '0.75rem' }}>
                          Semantic Heading Tree (H1–H4)
                        </Typography>
                      </Stack>
                    </Grid>
                    <Grid size={{ xs: 6 }}>
                      <Stack direction="row" spacing={0.75} sx={{ alignItems: 'center' }}>
                        <TableChartOutlinedIcon sx={{ fontSize: 16, color: 'accent.main' }} />
                        <Typography variant="caption" sx={{ fontWeight: 600, fontSize: '0.75rem' }}>
                          Structured Tables & Scope
                        </Typography>
                      </Stack>
                    </Grid>
                    <Grid size={{ xs: 6 }}>
                      <Stack direction="row" spacing={0.75} sx={{ alignItems: 'center' }}>
                        <FunctionsIcon sx={{ fontSize: 16, color: 'accent.main' }} />
                        <Typography variant="caption" sx={{ fontWeight: 600, fontSize: '0.75rem' }}>
                          STEM Formulas & MathML
                        </Typography>
                      </Stack>
                    </Grid>
                    <Grid size={{ xs: 6 }}>
                      <Stack direction="row" spacing={0.75} sx={{ alignItems: 'center' }}>
                        <RecordVoiceOverOutlinedIcon sx={{ fontSize: 16, color: 'accent.main' }} />
                        <Typography variant="caption" sx={{ fontWeight: 600, fontSize: '0.75rem' }}>
                          Meaningful Alt Text
                        </Typography>
                      </Stack>
            ...
            >
              ...
              <Box>
                ...
                <Typography variant="caption" sx={{ color: 'text.secondary', fontSize: '0.75rem', fontWeight: 500 }}>
                  High-Volume Scalability with Human QA
                </Typography>
              </Box>
            </GlassCard>
